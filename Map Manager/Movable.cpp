...
int Movable::pathfinding(sf::Vector2i pos, int **gameMap, int mapSize)
{
    ...
    while (!cells.empty())
    {
        ...
        if (validCell(sf::Vector2i(cur.x, cur.y - 1), gameMap, mapSize, weight))
        {
            if (sf::Vector2i(cur.x, cur.y - 1) == mPos)
                return (1);
            gameMap[cur.y - 1][cur.x] = weight + 1;
            cells.push(sf::Vector2i(cur.x, cur.y - 1));
        }
        if (validCell(sf::Vector2i(cur.x, cur.y + 1), gameMap, mapSize, weight))
        {
            if (sf::Vector2i(cur.x, cur.y + 1) == mPos)
        ...
        if (validCell(sf::Vector2i(cur.x - 1, cur.y), gameMap, mapSize, weight))
        {
            if (sf::Vector2i(cur.x - 1, cur.y) == mPos)
                return (1);
            gameMap[cur.y][cur.x - 1] = weight + 1;
            cells.push(sf::Vector2i(cur.x - 1, cur.y));
        }
        if (validCell(sf::Vector2i(cur.x + 1, cur.y), gameMap, mapSize, weight))
        {
            if (sf::Vector2i(cur.x + 1, cur.y) == mPos)
